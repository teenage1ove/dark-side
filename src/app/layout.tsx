...
export default function RootLayout({
  ...
  return (
    <html lang="en" style={{scrollBehavior: 'smooth'}}>
      <body className={inter.className} style={{backgroundImage: `url(${background.src})`}}>
        <div className='container m-auto max-sm:px-5 max-sm:text-[12px]'>
          <header className='flex justify-between items-center sticky top-5 my-4'>
            <div>
            ...
            <Link href='#articles'><button className='font-bold px-4 py-2 rounded-full bg-white text-black transition-all duration-500 hover:text-white hover:bg-transparent hover: drop-shadow-2xl hover:scale-125'> Делать деньги</button></Link>
          </header>
          {children}
        </div>
        </body>
